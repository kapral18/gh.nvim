...
function M.on_refresh()
    for issue, _ in pairs(M.state_by_number) do
        M.load_issue(issue, function()
            M.render_issue(issue)
        end)
    end
end
...
local function setup_buffer(number)
    ...
    vim.api.nvim_create_autocmd({ 'CursorMoved', 'CursorMovedI' }, {
        buffer = buf,
        callback = function()
            in_editable_area(M.state_by_number[number])
        end,
    })
...
function M.render_issue(number)
    ...
    table.insert(
        buffer_lines,
        string.format('%s  Author: %s', config.icon_set['Account'], state.issue['user']['login'])
    )
    table.insert(lines_to_highlight, { #buffer_lines, hi })
    ...
    table.insert(
        buffer_lines,
        string.format('%s  Last Updated: %s', config.icon_set['Calendar'], state.issue['updated_at'])
    )
    table.insert(lines_to_highlight, { #buffer_lines, hi })
    ...
    table.insert(
        buffer_lines,
        string.format('%s  Assigned:%s', config.icon_set['Account'], extract_issue_assignees(state.issue))
    )
    table.insert(lines_to_highlight, { #buffer_lines, hi })
    table.insert(
        buffer_lines,
        string.format('%s  Labels:%s', config.icon_set['Bookmark'], extract_issue_labels(state.issue))
    )
    table.insert(lines_to_highlight, { #buffer_lines, hi })
    ...
    for _, l in ipairs(lines_to_highlight) do
        vim.api.nvim_buf_set_extmark(buf, state.hi, l[1] - 1, 0, {
            line_hl_group = l[2],
        })
    end
    ...
    local out = ghcli.update_issue_body_async(state.issue['number'], body, function(err, _)
        if err then
            vim.schedule(function()
                lib_notify.notify_popup_with_timeout('Failed to update issue body: ' .. err, 7500, 'error')
            end)
            return
        end
