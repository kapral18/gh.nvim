...
local function setup_diff_ui()
    ...
    for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
        if not lib_util_win.is_component_win(vim.api.nvim_get_current_tabpage(), win) and cur_win ~= win then
    ...
    for _, win in ipairs(wins) do
        if win ~= wins[1] and vim.api.nvim_win_is_valid(win) then
...
local function parse_diff_hunk_line(line)
    if string.sub(line, 1, 1) == '+' or string.sub(line, 1, 1) == '-' or string.sub(line, 1, 1) == ' ' then
...
local function organize_threads(threads)
    ...
    for _, thread in ipairs(threads) do
        ...
        if thread.thread['originalLine'] ~= vim.NIL then
            local line =
                vim.api.nvim_buf_get_lines(buf, thread.thread['originalLine'] - 1, thread.thread['originalLine'], false)
            if line[1] == last_line then
...
function M.on_refresh()
    ...
    if
        state == nil
        or state.lwin == nil
        or not vim.api.nvim_win_is_valid(state.lwin)
        or state.rwin == nil
        or not vim.api.nvim_win_is_valid(state.rwin)
    then
        return
    end
    ...
    if state.displayed_thread ~= nil then
        local t_buf = thread_buffer.render_thread(
            state.displayed_thread.thread_id,
            state.thread_id_to_line[state.displayed_thread.thread_id],
            state.displayed_thread.n_of,
            state.displayed_thread,
            state.displayed_thread.side
        )
...
function M.toggle_thread_popup()
    ...
    if state.displayed_thread ~= nil and state.displayed_thread.popup then
    ...
    local buf =
        thread_buffer.render_thread(thread.thread['id'], state.thread_id_to_line[thread.thread['id']], { 1, #threads })
    if buf == nil then
        return
    end
    ...
    local popup_conf = vim.lsp.util.make_floating_popup_options(width, line_count, {
        border = 'rounded',
        focusable = true,
        zindex = 99,
        relative = 'cursor',
...
function M.next_thread()
    ...
    thread_buffer.render_thread(
        thread.thread['id'],
        state.thread_id_to_line[thread.thread['id']],
        { cur_thread_idx, #threads },
        nil,
        side
    )
...
function M.toggle_threads(thread_id)
    ...
    if state.displayed_thread ~= nil then
        if state.displayed_thread.side == side and state.displayed_thread.linenr == cursor[1] then
            if state.displayed_thread.popup == true then
                vim.api.nvim_win_close(state.displayed_thread.win, true)
            end
    ...
    local buf = thread_buffer.render_thread(
        thread.thread['id'],
        state.thread_id_to_line[thread.thread['id']],
        { n, #threads },
        nil,
...
function M.toggle_file_viewed()
    ...
    toggle_viewed_state(s.pull_state.pr_raw['node_id'], state.file['filename'], function()
        vim.cmd('GHRefreshPR')
    end)
end
