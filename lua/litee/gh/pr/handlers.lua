...
function M.ui_handler(refresh, on_load_ui)
    ...
    if not refresh then
        ...
        if gitcli.repo_dirty() then
            lib_notify.notify_popup_with_timeout(
                'Cannot open pull request because repository has changes. Stash changes and try again.',
                7500,
                'error'
            )
            return
        ...
        if remote_branch_exists then
            ...
            lib_notify.notify_popup_with_timeout(
                'Remote branch no longer exists, cannot fetch it. PR will be opened but certain commits may not be available for checkout.',
                7500,
                'warning'
            )
        end
    ...
    local pr_root = lib_tree_node.new_node(s.pull_state.number, s.pull_state.number, 0)
    ...
    local cursor = nil
    if global_state['pr'].win ~= nil and vim.api.nvim_win_is_valid(global_state['pr'].win) then
        cursor = vim.api.nvim_win_get_cursor(global_state['pr'].win)
    end

    if state_was_nil then
        ...
        lib_tree.write_tree_no_guide_leaf(state.buf, state.tree, marshaller.marshal_pr_node)
    end
...
function M.pr_handler(pull_number, refresh, on_load_ui)
    pr_state.load_state_async(
        pull_number,
        vim.schedule_wrap(function()
            M.ui_handler(refresh, on_load_ui)
        end)
    )
end
...
function M.review_handler(review_id, refresh)
    ...
    local root = lib_tree_node.new_node(review['id'], review['id'], 0)
    root.review = review
    root.expanded = true
    ...
    for _, t in pairs(s.pull_state.review_threads_by_id) do
        if t.thread['review_id'] ~= nil and t.thread['review_id'] == review_id then
            -- do a shallow clone of threads, only updating the depth scaler but
            -- not copying larger data structures.
    ...
    -- and buffer?
    local cursor = nil
    if global_state['pr_review'].win ~= nil and vim.api.nvim_win_is_valid(global_state['pr_review'].win) then
...
function M.commits_handler(sha, refresh)
    ...
    if not refresh then
        ...
        if gitcli.repo_dirty() then
            lib_notify.notify_popup_with_timeout(
                'Cannot checkout selected commit because repository has changes. Stash changes and try again.',
                7500,
                'error'
            )
            return
    ...
    local root = lib_tree_node.new_node(commit['sha'], commit['sha'], 0)
    root.commit = commit
    root.location = nil
    ...
    for i, file in ipairs(commit['files']) do
        local child_node = lib_tree_node.new_node(file['filename'], file['filename'], 1)
    ...
    -- and buffer?
    local cursor = nil
    if global_state['pr_files'].win ~= nil and vim.api.nvim_win_is_valid(global_state['pr_files'].win) then
    ...
    if state_was_nil then
        ...
        lib_tree.write_tree_no_guide_leaf(state.buf, state.tree, marshaller.marshal_pr_file_node)
    end
...
local function on_refresh()
    ...
    if comp_state ~= nil and comp_state.tree ~= nil then
